...
def payment_as_receiver(three_addresses, sender_actor, payment_action):
    _, _, a0 = three_addresses
    subaddr = LibraAddress.from_bytes("dm", a0.onchain_address_bytes, b'x'*8)
...
def json_request(my_addr, other_addr, payment_action):
    sub_sender = LibraAddress.from_bytes("dm", my_addr.onchain_address_bytes, b'a'*8)
    sub_receiver = LibraAddress.from_bytes("dm", other_addr.onchain_address_bytes, b'b'*8)
...
async def test_vasp_simple_wrong_VASP(json_request, other_addr, loop, key):
    my_addr = LibraAddress.from_bytes("dm", b'X'*16)
