...
def test_send_request(network_client, httpserver, request_json, response_json):
    url = '/process'
    httpserver.expect_request(url).respond_with_json(response_json)
    network_client.send_request(httpserver.url_for(url), request_json)


def test_send_request_unknown_receiver(network_client, request_json):
    url = 'http://bad_url'
    network_client.send_request(url, request_json)
