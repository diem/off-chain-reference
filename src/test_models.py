...
def test_protocol_server_client_benign():
    ...
    server.sequence_command_local('Hello')
    msg_list = server.tap()
    ...
    assert server.final_sequence[0].response is None
    server.handle_response(reply)
    msg_list = server.tap()
    assert len(msg_list) == 0 # No message expected

    assert server.final_sequence[0].response is not None
    assert client.final_sequence[0].response is not None
    assert client.final_sequence[0].command == 'Hello'
...
def test_protocol_server_conflicting_sequence():
    ...
    request_conflict.command = "Conflict"
...
def test_protocol_client_server_benign():
    ...
    # Create a server request for a command
    client.sequence_command_local('Hello')
    msg_list = client.tap()
    assert len(msg_list) == 1
    request = msg_list.pop()
    assert isinstance(request, CommandRequestObject)
...
def test_protocol_server_client_interleaved_benign():
    ...
    server.sequence_command_local('World')
    ...
    #print([c.command for c in client.final_sequence])
    #print([c.command for c in server.final_sequence])
    ...
    assert len(client.final_sequence) == 2
    assert len(server.final_sequence) == 2
    assert [c.command for c in client.final_sequence] == ['World', 'Hello']
    assert [c.command for c in server.final_sequence] == ['World', 'Hello']
...
def test_random_interleave_no_drop():
    ...
    client_seq = [c.command for c in client.final_sequence]
    server_seq = [c.command for c in server.final_sequence]
